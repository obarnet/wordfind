    ...
    var endTurn = function () {
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
        ...
        if (wordList.length === 0) {
		  ...
		  // Show congratulations overlay, final score and friends scores
		  // If pressed next
		    var words = ['manje', 'vòlè', 'anmèdan', 'raketè', 'djòlè', 'sezon',
               'akdenesans', 'kreyòl', 'randevou', 'fanatik', 'batistè',
			   'ras', 'almanak', 'chevalye', 'kondane', 'rivyè', 'andeyò',
			   'anvan', 'apre', 'defisi', 'benefis', 'alonj', 'plenn',
			   'plenyen', 'debat', 'netwaye'
               ];
		  // start a word find game
		  var gamePuzzle = window.wordfindgame.create(words, '#puzzle', '#words');
        }
      }
      ...
      */
      create: function(words, puzzleEl, wordsEl, options) {
        ...
        if (window.navigator.msPointerEnabled) {
		  ...
		  $('#puzzle').on('MSPointerOut', mouseLeave);
        }
        else {
